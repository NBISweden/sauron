...
for( t in names(obj_list) ){
  ...
  for(m in obj_list[[t]]){
    ...
    if(file.exists( paste0(opt$output_path,"/Marker_list_",t,"_",obj_list[[t]],"_",paste0(clust_use,collapse = "_"),".csv"))){
      cat("\nMarker_list found and will be used:\n", paste0(opt$output_path,"/Marker_list_",t,"_",obj_list[[t]],"_",paste0(clust_use,collapse = "_"),".csv"),"\n")
      temp_markers2 <- read.csv2( paste0(opt$output_path,"/Marker_list_",t,"_",obj_list[[t]],"_",paste0(clust_use,collapse = "_"),".csv"),row.names = 1,stringsAsFactors = F) 
    } else {
      ...
      write.csv2(temp_markers2 , paste0(opt$output_path,"/Marker_list_",t,"_",obj_list[[t]],"_",paste0(clust_use,collapse = "_"),".csv"),row.names = T)
    }
...
write.csv2(marker_list_all , paste0(opt$output_path,"/Full_Marker_list.csv"),row.names = T)
write.csv2(marker_list_L , paste0(opt$output_path,"/Full_marker_list_L.csv"),row.names = T)
write.csv2(marker_list_R , paste0(opt$output_path,"/Full_marker_list_R.csv"),row.names = T)
try(rm(temp,temp2))
invisible(gc())
#---------
...
write.csv2(det_L_R_pairs , paste0(opt$output_path,"/Connected_pairs_genes.csv"),row.names = T)
write.csv2(marker_list_uniqueness , paste0(opt$output_path,"/graph_edges_with_uniqueness.csv"),row.names = T)
#---------
...
dev.off()
saveRDS(g,paste0(opt$output_path,"/Graph_all_ligand_receptor.rds") )
#---------
...
dev.off()
saveRDS(g,paste0(opt$output_path,"/Graph_paired_ligand_receptor_uniqueness.rds") )
#---------
...
if(!dir.exists(paste0(opt$output_path,"/Individual"))){dir.create(paste0(opt$output_path,"/Individual"),recursive = T)}
for( i in datasets){
  ...
  png(filename = paste0(opt$output_path,"/Individual/Graph_paired_ligand_receptor_metadata_cluster_",i,"_all_",ifelse(grepl("L",i),"receptors","ligands"),".png"),width = 1800,height = 1800*max(.6, length(unique(unlist(k)))/100 ),res = 150)
  ...
  if(grepl("L",i)){
    for( j in opposite_dtsts){
      ...
      png(filename = paste0(opt$output_path,"/Individual/Graph_paired_ligand_receptor_metadata_cluster_",i,"_",j,".png"),width = 1800,height = 1800*max(.6, length(unique(unlist(k)))/100 ),res = 150)
      plot.igraph(g_temp, vertex.label.color="black",vertex.label.family="sans",vertex.label.cex=1,vertex.label.font=2,
                  vertex.shape="vrectangle", vertex.size=25,vertex.size2=4/max(.6, length(unique(unlist(k)))/100 ),edge.arrow.width=edge_scalar*4,edge.arrow.size=0,
                  vertex.color=paste0(cols,90 ), vertex.frame.color=cols ,layout=-l,
                  edge.color=mycolor,edge.width=edge_scalar,asp = max(.6, length(unique(unlist(k)))/100 )  )
