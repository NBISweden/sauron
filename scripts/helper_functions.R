...
#---------------
draw_violin <- function(x,method="log",plot_points=T,points_method="proportional",col="grey",border="grey",at=1,pt.col="grey",bw=0.45,max_points=200){
  r <- sum(x!=0)/length(x)
  if(plot_points){
  ...
  if(method == "log"){
    x2 <- log2(log2(x+1)+1)
    a <- density(x2,bw = .05,n = 200)
    ys <- 2^(2^a$x-1)-1
    xs <- a$y/max(a$y)*bw
  }
  
  if(method == "mixed"){
    d1 <- density(x[x==0],bw = .01,n = 200)
